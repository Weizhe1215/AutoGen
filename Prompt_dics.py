Strategy_requirements_dic = {
    "Simple Quant Strategy": "以下是量化策略的模板，以下内容都需要在段落中进行体现："
                             ...
                             "5. 管理⼈的量化模型，需要明确提示是线性还是机器学习模型或者是两者都有及其偏好，详述他们的建模方法论，包括使用了何种"
                             "模型进行了什么工作"
                             "6. 管理⼈的因⼦⼊库条件, 即管理人的新因子入库标准和流程 "
                             "7. 管理⼈的具体执⾏, 对股票进行了什么处理，有无使用算法下单等，最终持有了多少个股，指数内占比，年化双边换手率多少倍。"
...
Strategy_prompt_dic = {
    "Simple Quant Strategy": [
        ...
        '从文件中提取出管理人的具体操作，例如有无使用自研/外部的下单算法，不同策略的持股数是多少，年化双边换手率多少倍等'
    ]
}
